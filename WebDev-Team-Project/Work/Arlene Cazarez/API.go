...
func userNameExists(res http.ResponseWriter, req *http.Request){
	...
	if(err != nil){
		...
		// If the user was retrieved day so.
		fmt.Fprint(res ,"exists")
	}
}

// =======================================================================
// An API call that checks if a password is good given a username.
// Used by AJAX through a GET at "/usernameexists"
// =======================================================================
func passwordIsGood(res http.ResponseWriter, req *http.Request){
	// ================================
	// No need to check an empty username field.
	// ================================
	...
	res.Header().Set("Content-Type","text/plain")
	if(err != nil){
		// IF the user doesn't exist obviously the password is bad.
		fmt.Fprint(res ,"doesn't exist")
	} else if(data.Password != req.FormValue("password")){
		// If the password is bad say so.
		fmt.Fprint(res ,"bad password")
	} else if(data.Password == req.FormValue("password")){
		// Otherwise the password should be good.
		fmt.Fprint(res ,"good password")
	}
}
...
func emailInUse(res http.ResponseWriter, req *http.Request){
	if(req.FormValue("email") == ""){
		fmt.Fprint(res,"email already in use")
		return
	}
	// Grab Emails
	...
	fmt.Fprint(res,"email not in use")
}
...
func registerRequest(res http.ResponseWriter, req *http.Request){
	...
	if err != nil {
		...
		// Add user to 'pages'
		pages, err := retrievePages(req)
		if err != nil {}
		pages.Data = append(pages.Data,req.FormValue("username"))
		err = storePages(pages,req)
		if err != nil { return }
		// ====================
